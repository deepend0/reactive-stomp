...
public class WebSocketBrokerIntegrationTest {
    private static final Logger LOGGER = LoggerFactory.getLogger(WebSocketBrokerIntegrationTest.class);
    ...
    @Test
    public void shouldConnectMessageToMultipleClients() {
    ...
    private void receiveMessage(Deque<byte[]> receivedMessages, String subId, String destination, String message) {
        ...
        Awaitility.await().atMost(Duration.ofMillis(5000)).pollInterval(Duration.ofMillis(1000)).until(() -> !receivedMessages.isEmpty());
        byte[] first = receivedMessages.removeFirst();
        Matcher matcher = Pattern.compile(new String(messageFrame)).matcher(new String(first));
    ...
    private void sendMessage(WebSocketClientConnection connection, Deque<byte[]> receivedMessages, String destination, String message, String receipt) {
        ...
        Awaitility.await().atMost(Duration.ofMillis(3000)).pollInterval(Duration.ofMillis(300)).until(() -> !receivedMessages.isEmpty());
        byte [] first = receivedMessages.removeFirst();
    ...
    private void connectClient(WebSocketClientConnection connection, Deque<byte[]> receivedMessages, Deque<byte[]> receivedHeartbeats) {
        ...
        Assertions.assertTrue(matcher.matches());
        Awaitility.await().atMost(Duration.ofMillis(3000)).pollInterval(Duration.ofMillis(300)).until(() -> receivedHeartbeats.size() > 1);
    }
